#!/usr/bin/env python
# coding: utf-8
"""Measurement-outcome-1 Unitary-PQC overparameterization pipeline.
...
analysis boundaries.
At the end of every layer, the feed-forward unitary
``Rz(varphi) Rx(2 phi) Rz(varphi)`` is applied to qubit 4. Numerical plots are
handled by ``unitary_pqc_measured_1_overparam_visualize.py``, and circuit
drawings by ``unitary_pqc_measured_1_overparam_draw_circuits.py``.

Example::

    python unitary_pqc_measured_1_overparam_compute.py --h-param 0.1 --vqe-batch-size 20 --analysis-batch-size 1
"""
...
def _parse_cli_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Run the measurement-outcome-1 Unitary-PQC numerical pipeline. "
...
def hessian_rank_and_condition_from_eigvals(eigvals: jnp.ndarray):
    ...
    abs_eigvals = jnp.abs(jnp.asarray(eigvals, dtype=REAL_DTYPE))
    threshold = jnp.asarray(QFIM_EFFECTIVE_RANK_THRESHOLD, dtype=REAL_DTYPE)
    active = abs_eigvals >= threshold
    rank_value = jnp.sum(active, dtype=jnp.int64)
    largest_active = jnp.max(jnp.where(active, abs_eigvals, 0.0))
    smallest_active = jnp.min(jnp.where(active, abs_eigvals, jnp.inf))
...
def make_hessian_analysis_batch_runner(num_layers: int):
    """Create the random-point Hessian rank/condition batch runner."""
...
def run_random_qfim_analysis(
    *,
    ...
    for L in tqdm(qfim_layer_list, desc="Layers (QFIM)", unit="layer"):
        ...
        # --------------------------
        # Energy Hessian at the same random points used for the QFIM.
    ...
    save_npz_result(
        ...
        analysis_kind=np.asarray("random_points"),
        ansatz=np.asarray(ANSATZ_NAME),
        measurement_outcome=np.asarray(MEASUREMENT_OUTCOME, dtype=NP_INT_DTYPE),
        h_param=np.asarray(h_param, dtype=NP_REAL_DTYPE),
        layers=np.asarray(qfim_layer_list, dtype=NP_INT_DTYPE),
