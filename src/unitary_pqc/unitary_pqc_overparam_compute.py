...
Example::
    ...
    python unitary_pqc_overparam_compute.py --h-param 0.1 --vqe-batch-size 20 --analysis-batch-size 1
"""
...
def _parse_cli_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Run the Unitary-PQC numerical pipeline. By default VQE and the "
...
if __name__ == "__main__":
    ...
    if _CLI_ARGS.stage == "all":
        # Exit before importing JAX/TensorCircuit below.  Only the two child
...
def hessian_rank_and_condition_from_eigvals(
    eigenvalues: jnp.ndarray,
    *,
    threshold: Optional[float] = None,
) -> tuple[jnp.ndarray, jnp.ndarray]:
    """Return inclusive absolute rank and active-spectrum condition number."""
    abs_eigs = jnp.abs(jnp.real(jnp.asarray(eigenvalues)))
    threshold_value = (
        QFIM_EFFECTIVE_RANK_THRESHOLD
        if threshold is None
        else float(threshold)
    )
    threshold_array = jnp.asarray(threshold_value, dtype=abs_eigs.dtype)
    active = abs_eigs >= threshold_array
    rank_value = jnp.sum(active)
    largest_active = jnp.max(jnp.where(active, abs_eigs, 0.0))
    smallest_active = jnp.min(jnp.where(active, abs_eigs, jnp.inf))
...
def make_hessian_analysis_batch_runner(num_layers: int):
    """Return random-point Hessian rank and active-spectrum condition number.

    The fixed QFIM threshold is applied inclusively to absolute Hessian
    eigenvalues.  A rank-zero Hessian has no active spectrum, so its condition
    number is represented by NaN.
    """
    eigvals_fn = make_energy_hessian_eigvals_fn_for_layer(int(num_layers))

    def metrics_one(theta: jnp.ndarray):
        return hessian_rank_and_condition_from_eigvals(
            eigvals_fn(theta),
...
def load_unitary_vqe_samples(inpath: Optional[str] = None) -> str:
    ...
    with np.load(result_path, allow_pickle=False) as data:
        ...
        if (
            ...
            raise ValueError(
                "Saved h_param, steps, and num_runs must be scalar values."
            )
        if (
...
def run_random_qfim_analysis(
    *,
    ...
    for L in tqdm(qfim_layer_list, desc="Layers (QFIM)", unit="layer"):
        ...
        #   Only rank and the active-spectrum condition number are retained.
        # --------------------------
        hessian_batch_runner = make_hessian_analysis_batch_runner(L)
        hessian_ranks, hessian_conditions = _evaluate_analysis_in_batches(
            qfim_random_thetas_by_layer[L],
...
def _run_selected_cli_stage(args: argparse.Namespace) -> int:
    ...
    if args.stage == "qfim":
        ...
        print(
            ...
            "circuits, run "
            "src/unitary_pqc/unitary_pqc_overparam_draw_circuits.py "
            "separately."
        )
        return 0
