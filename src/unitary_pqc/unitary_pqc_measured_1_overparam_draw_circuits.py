...
Examples::
    ...
    python src/unitary_pqc/unitary_pqc_measured_1_overparam_draw_circuits.py
    python src/unitary_pqc/unitary_pqc_measured_1_overparam_draw_circuits.py --layers 1 4 8
    python src/unitary_pqc/unitary_pqc_measured_1_overparam_draw_circuits.py --layers 8 --fold -1
"""
...
def _parse_cli_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Draw optimized measurement-outcome-1 Unitary-PQC circuits from "
            "saved VQE results; "
...
def feed_forward_measured_1(
    ...
    """Append the outcome-1 feed-forward rotation to the center qubit q4."""
    circuit.rz(float(varphi), ANCILLA_QUBIT)
    circuit.rx(float(2.0 * phi), ANCILLA_QUBIT)
    circuit.rz(float(varphi), ANCILLA_QUBIT)


def create_unitary_pqc(theta: np.ndarray, num_layers: int) -> QuantumCircuit:
    """Rebuild the optimized outcome-1 five-qubit circuit for drawing."""
    ...
    for layer_theta in theta_layers:
        blocks = layer_theta[:-FEED_FORWARD_PARAMS_PER_LAYER].reshape(
            NUM_BLOCKS,
            PARAMS_PER_BLOCK,
        )
        for (q0, q1), params in zip(LAYER_PAIRS, blocks):
            qg_layer(circuit, q0, q1, params)
        feed_forward_measured_1(
            circuit,
            layer_theta[-2],
            layer_theta[-1],
        )
