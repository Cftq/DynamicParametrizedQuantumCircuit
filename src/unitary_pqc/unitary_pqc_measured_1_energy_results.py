#!/usr/bin/env python
# coding: utf-8
"""Read saved outcome-1 U3-Cartan VQE energies without a quantum runtime.
...
def load_measured_unitary_final_energies(
    ...
    *,
    ...
    The archive must belong to the outcome-1, 62-parameter-per-layer model
    ...
    if not archive_path.is_file():
        raise FileNotFoundError(
            f"Saved Measured 1 VQE energy histories were not found: {archive_path}"
        )
    ...
    with np.load(archive_path, allow_pickle=False) as archive:
        ...
        outcome = _positive_integer_scalar(
            archive["measurement_outcome"], name="VQE measurement_outcome"
        )
        if outcome != 1:
            raise ValueError(f"VQE measurement_outcome mismatch: {outcome} != 1.")
